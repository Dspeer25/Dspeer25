...
export default function NavBar({ view, tabs, activeTab, onTabClick, onLogoClick, showClickHint = false, tabGlow = false, profileTabGlow = false, traderProfileTabRef }: NavBarProps) {
  return (
    <>
      ...
      <nav style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "0 48px", minHeight: 80, borderBottom: "1px solid #1a1b22", overflow: "visible", position: 'relative' }}>
        ...
        <span style={{ position: 'absolute', top: 32, right: 48, color: teal, fontFamily: fm, fontSize: 15, cursor: 'pointer', fontWeight: 500 }}>Login</span>
        <div style={{ display: "flex", gap: 5, width: "100%", maxWidth: 920 }}>
          {tabs.map(t => (
            <span
              ...
              style={{
                fontSize: 14, color: teal, letterSpacing: "0.04em", padding: "14px 32px", cursor: "pointer", fontFamily: fm, borderRadius: "8px 8px 0 0", fontWeight: 600,
                background: view === 'app' && activeTab === t ? "rgba(0,212,160,0.18)" : "rgba(0,212,160,0.05)",
                borderTop: view === 'app' && activeTab === t ? `1px solid ${teal}` : "1px solid rgba(0,212,160,0.25)",
                borderRight: view === 'app' && activeTab === t ? `1px solid ${teal}` : "1px solid rgba(0,212,160,0.25)",
                borderBottom: "none",
                borderLeft: view === 'app' && activeTab === t ? `1px solid ${teal}` : "1px solid rgba(0,212,160,0.25)",
