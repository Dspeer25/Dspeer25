...
export default function AISidebar() {
  ...
  const [messages, setMessages] = useState([{ role: 'coach', text: "Hello. I've reviewed your last 18 trades. Your breakout setups are your edge — 71% win rate. Your biggest leak is mean reversion at 28%. Consider removing it from your playbook entirely." }])
  ...
  const sendMessage = async () => {
    ...
    } catch { setMessages(prev => [...prev, { role: 'coach', text: 'Something went wrong. Try again.' }]) }
    setLoading(false)
  }

  const S = ({ size = 24, color = '#00d4a0' }: { size?: number; color?: string }) => (
      ...
      {isOpen && (
        <div onClick={(e) => e.stopPropagation()} style={{position:'fixed',right:0,top:0,height:'100vh',width:320,zIndex:999999,background:'#0a0a0a',borderLeft:'0.5px solid #1e1e1e',display:'flex',flexDirection:'column',fontFamily:'DM Mono,monospace'}}>
          <div style={{padding:'16px',borderBottom:'0.5px solid #1a1a1a',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            ...
            <button onClick={() => setIsOpen(false)} style={{background:'none',border:'none',color:'#555',fontSize:20,cursor:'pointer',lineHeight:1}}>×</button>
          </div>
          ...
          <div style={{padding:'12px 16px',borderBottom:'0.5px solid #1a1a1a'}}>
            ...
            <div style={{background:'#0c1812',border:'0.5px solid rgba(0,212,160,0.15)',borderRadius:6,padding:'10px 12px'}}>
              <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}><S size={12}/><span style={{fontSize:10,color:'#00d4a0'}}>COACH</span></div>
              <div style={{fontSize:11,color:'#5a7a68',lineHeight:1.6}}>You&apos;ve exited early on 4 of your last 6 winners. This has cost an estimated <span style={{color:'#00d4a0'}}>$240</span> in unrealized gains this month.</div>
            </div>
          </div>
          <div style={{flex:1,overflowY:'auto',padding:'12px 16px',display:'flex',flexDirection:'column',gap:10}}>
            {messages.map((m,i)=>(
              <div key={i} style={{alignSelf:m.role==='user'?'flex-end':'flex-start',maxWidth:'85%'}}>
                {m.role==='coach'&&<div style={{display:'flex',alignItems:'center',gap:5,marginBottom:4}}><S size={10}/><span style={{fontSize:9,color:'#00d4a0'}}>COACH</span></div>}
            ...
            {loading&&<div style={{alignSelf:'flex-start'}}><div style={{background:'#0c1812',border:'0.5px solid rgba(0,212,160,0.1)',borderRadius:'6px 6px 6px 0',padding:'10px 14px',display:'flex',gap:4}}>{[0,1,2].map(i=><div key={i} className="dp" style={{width:5,height:5,borderRadius:'50%',background:'#00d4a0'}}/>)}</div></div>}
